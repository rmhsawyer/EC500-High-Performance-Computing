...
We solve the 2-d Laplace problem on an LxL lattice on both a periodic square lattice and  periodic triangular 
lattice:    A phi = b  

Square lattice (2 by 2 non-overlaping blocking )
...
At present relex iteration does Gauss Seidel. Can change to Gauss Jacobi or Red Black on the GPU.

Triangular lattice (redution of 4 by  overlapping blocks)

 (A phi)((x,y) = [6 phi(x,y) - phi(x+1,y) - phi(x-1,y) - phi(x,y+1)  -phi(x,y+1)  - phi(x-1,y+1)  -phi(x+1,y-1)]/a^2 + m^2 phi(x,y) = b(x,y)

with x,y oblique lattice 
...
{  
  ...
  p.isTriangle = true;
  // p.Lmax = 9; // max number of levels
  p.Lmax = 1;
  p.N = 2*(int)pow(2,p.Lmax);  // MUST BE POWER OF 2
   //p.m = 0.1;
  if(p.isTriangle) {  p.m = 0.01*sqrt(3.0/2.0);}
  else  {  p.m = 0.01;}
  ...
  printf("\n For triange %d V cycle for %d by %d  with nlev = %d out of max  %d \n",p.isTriangle, p.N, p.N, nlev, p.Lmax); 
  
  // initialize arrays__________________________________
  p.size[0] = p.N;
  p.a[0] = 1.0;
  
  if(p.isTriangle) {  p.scale[0] = 1.0/(6.0 + p.m*p.m);}
  else { p.scale[0] = 1.0/(4.0 + p.m*p.m);}
  ...
  for(lev = 1;lev< p.Lmax+1; lev++) {
    ...
    p.a[lev] = 2.0 * p.a[lev-1];
     ...
     if(p.isTriangle) {
       // p.scale[lev] = 1.0/(6.0 + p.m*p.m);
      p.scale[lev] = 1.0/(6.0 + p.m*p.m*p.a[lev]*p.a[lev]);
     }
     else
       {
      //  p.scale[lev] = 1.0/(6.0 + p.m*p.m*p.a[lev]*p.a[lev]);
  ...
  while(resmag > 0.00001)
	...
	{    
      ...
      //   relax(phi[nlev],res[nlev],nlev, n_per_lev,p);   // lev = nlev -1, ... 0;
      
      for(lev = nlev;lev >= 0; lev--)  //come up
	{
	   relax(phi[lev],res[lev],lev, n_per_lev,p);   // lev = nlev -1, ... 0;  
	      if(lev > 0) inter_add(phi[lev-1], phi[lev], lev, p); // phi[lev-1] += error = P phi[lev] & set phi[lev] = 0;
	}
      resmag = GetResRoot(phi[0],res[0],0,p);
      printf("At the %d cycle the mag residue is %g \n",ncycle,resmag);
    }
...
{  
  ...
  for(i=0; i<niter; i++)    
      ...
      for(y = 0; y < L; y++)
	{
	  ...
	  phi[x + y*L] += p.scale[lev] * ( phi[x + ((y+1)%L)*L]  + phi[x + ((y-1+L)%L)*L]);
	  if(p.isTriangle) phi[x + y*L] += p.scale[lev] *(phi[(x+1)%L + ((y+1)%L)*L] + phi[(x-1+L)%L + ((y-1+L)%L)*L]);
	}
...
{  
  ...
  for(x = 0; x< L; x++)
	...
	if(p.isTriangle) r[x + y*L] += p.scale[lev] *(phi_f[(x+1)%L + ((y+1)%L)*L] + phi_f[(x-1+L)%L + ((y-1+L)%L)*L]);
      }
  ...
  for(x = 0; x< Lc; x++)
	...
	if(p.isTriangle)
	  {
	    res_c[x + y*Lc] = 0.5* r[2*x + 2*y*L];
	    res_c[x + y*Lc] += (1.0/12.0)* (r[(2*x + 1)%L + 2*y*L] + r[(2*x - 1 +L)%L + 2*y*L]);
	    res_c[x + y*Lc] += (1.0/12.0)* (r[2*x + ((2*y+1)%L)*L] + r[2*x + ((2*y-1+L)%L)*L]);
	    res_c[x + y*Lc] +=  (1.0/12.0)* (r[(2*x + 1)%L + ((2*y+1)%L)*L] + r[(2*x - 1 +L)%L + ((2*y - 1 +L)%L)*L]);
	  }
	else
	  {
...
{  
  ...
  for(x = 0; x< Lc; x++)
	...
	phi_f[(2*x +1)%L + 2*y*L]       += 0.5*phi_c[x + y*Lc];
	phi_f[(2*x - 1 +L)%L + 2*y*L]   += 0.5*phi_c[x + y*Lc];
	
	phi_f[2*x + ((2*y* + 1)%L)*L]     += 0.5*phi_c[x + y*Lc];
	phi_f[2*x + ((2*y - 1 + L)%L)*L]  += 0.5*phi_c[x + y*Lc];
	
	phi_f[(2*x +1)%L + ((2*y +1)%L)*L]         += 0.5*phi_c[x + y*Lc];
	phi_f[(2*x - 1 +L)%L + ((2*y -1 + L)%L)*L] += 0.5*phi_c[x + y*Lc];
      }
...
{ //true residue
  ...
  for(x = 0; x < L; x++)
	...
	+ (phi[(x+1)%L + y*L] + phi[(x-1+L)%L + y*L] 
      ...
      if(p.isTriangle) residue += (phi[(x+1)%L + ((y+1)%L)*L] + phi[(x-1+L)%L + ((y-1+L)%L)*L]);
